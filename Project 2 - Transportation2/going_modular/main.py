...
parser.add_argument("--zone", help="which zone are you looking at?", type=str,
                    choices=["SiouxFalls", "Anaheim", "Chicago_Sketch"])
...
parser.add_argument("-n", "--neural_network",
                    ...
                    type=str, default="")
parser.add_argument("--bin", help="set the sclae of histogram.[start, stop, step]", type=str,
                    default="-0.5,0.55,0.05")
...
if scale:
    ...
    if "guess" in gravity_model:
        ...
        model_guess.pass_data_from_folder_scaled(folder=f"data/{zone}/Scaled/random-{seed}", missing_rate=mis,
                                                 approach="guess",
                                                 scale_time_period=5,
                                                 saved_plot_folder=f"Plots/GM_Plots/{zone}_plots/Scaled/random_{seed}/guess",
                                                 saved_data_folder=f"Results/GM_Results/{zone}_results/Scaled/random_{seed}",
    ...
    if "exp" in gravity_model:
        ...
        model_exp.pass_data_from_folder_scaled(folder=f"data/{zone}/Scaled/random_{seed}", missing_rate=mis,
                                               approach="exp",
                                               saved_plot_folder=f"Plots/GM_Plots/{zone}_plots/Scaled/random_{seed}/exp",
                                               saved_data_folder=f"Results/GM_Results/{zone}_results/Scaled/random_{seed}",
                                               bins=np.arange(float(bin[0]), float(bin[1]), float(bin[2])))
    if "power" in gravity_model:
        ...
        model_power.pass_data_from_folder_scaled(folder=f"data/{zone}/Scaled/random_{seed}",
                                                 ...
                                                 saved_plot_folder=f"Plots/GM_Plots/{zone}_plots/Scaled/random_{seed}/power",
                                                 saved_data_folder=f"Results/GM_Results/{zone}_results/Scaled/random_{seed}",
                                                 bins=np.arange(float(bin[0]), float(bin[1]), float(bin[2])))
    if "tanner" in gravity_model:
        ...
        model_tanner.pass_data_from_folder_scaled(folder=f"data/{zone}/Scaled/random_{seed}", missing_rate=mis,
                                                  approach="tanner",
                                                  saved_plot_folder=f"Plots/GM_Plots/{zone}_plots/Scaled/random_{seed}/tanner",
                                                  saved_data_folder=f"Results/GM_Results/{zone}_results/Scaled/random_{seed}",
                                                  bins=np.arange(float(bin[0]), float(bin[1]), float(bin[2])))
    print("NNS")
    if neural_network:
        ...
        for iteration in mis:
            ...
            with torch.inference_mode():
                ...
                torch.save(model, f"Models/SiouxFall/SiouxFall{seed}_{int(iteration * 100)}_model_nn.pth")
    ...
    if "guess" in gravity_model:
        ...
        model_guess.pass_data_from_folder_not_scaled(folder=f"data/{zone}/Not_Scaled/random_{seed}", missing_rate=mis,
                                                     ...
                                                     saved_plot_folder=f"Plots/GM_Plots/{zone}_plots/Not_Scaled/random_{seed}/guess",
                                                     saved_data_folder=f"Results/GM_Results/{zone}_results/Not_Scaled/random_{seed}",
                                                     )
    if "exp" in gravity_model:
        model_exp = Js.GM()
        model_exp.pass_data_from_folder_not_scaled(folder=f"data/{zone}/Not_Scaled/random_{seed}",
                                                   missing_rate=mis,
                                                   approach="exp",
                                                   saved_plot_folder=f"Plots/GM_Plots/{zone}_plots/Not_Scaled/random_{seed}/exp",
                                                   saved_data_folder=f"Results/GM_Results/{zone}_results/Not_Scaled/random_{seed}")
    if "power" in gravity_model:
        model_power = Js.GM()
        model_power.pass_data_from_folder_not_scaled(folder=f"data/{zone}/Not_Scaled/random_{seed}",
                                                     missing_rate=mis,
                                                     approach="power",
                                                     saved_plot_folder=f"Plots/GM_Plots/{zone}_plots/Not_Scaled/random_{seed}/power",
                                                     saved_data_folder=f"Results/GM_Results/{zone}_results/Not_Scaled/random_{seed}")
    if "tanner" in gravity_model:
        model_tanner = Js.GM()
        model_tanner.pass_data_from_folder_not_scaled(folder=f"data/{zone}/Not_Scaled/random_{seed}", missing_rate=mis,
                                                      approach="tanner",
                                                      saved_plot_folder=f"Plots/GM_Plots/{zone}_plots/Not_Scaled/random_{seed}/tanner",
                                                      saved_data_folder=f"Results/GM_Results/{zone}_results/Not_Scaled/random_{seed}")
    ...
    if neural_network:
        ...
        for iteration in mis:
            temp = 0
            folder = f"data/{zone}/Not_Scaled/random_{seed}/{int(iteration * 10)}"
            data = ML.DataLoader_Me(folder, iteration, batch_size=int(hyper_parameters[1]), device="cpu",
                                    dr=args.drop_last)
            train, val, test = data.train, data.val, data.test
