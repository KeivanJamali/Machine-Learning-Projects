...
for dataset in city:
    ...
    for num in data_num:
        for rate in i:
            ...
            edge_weight = 0.0001 / torch.tensor([times], dtype=torch.float)
            ...
            data = data.to(device)
            model = Network(n_node_features=data.num_node_features, hidden_layer=4, node_embedding_size=2, model='GAT')
            model_name = 'GAT'
            model = model.to(device)
            link_regressor = MLP(n_node_features=2, hidden_channels=64)
            link_regressor = link_regressor.to(device)
            params = list(model.parameters()) + list(link_regressor.parameters())
            optimizer = torch.optim.Adam(params, lr=0.015, weight_decay=0.000001)
            loss = MSELoss()
            ...
            # plt.xlabel('loss valid')
            # plt.ylabel('epoch')
            # plt.title('epoch vs loss valid' + str(dataset) + str(int(rate) / int(100)) + str(model_name))
            # plt.legend()
            ...
            #     model_name))
            # plt.legend()
            # folder_path = 'E:/class/Ewh, University!/8/summer project/sina TDM/Hoor result' + '/' + 'Histogram of Actual - Predicted Differences' + model_name + '/' + str(
            #     dataset) + '/' + 'prepared_data-' + str(num)
            # os.makedirs(folder_path, exist_ok=True)
            # file_path = os.path.join(folder_path, str(dataset) + str(rate) + '.png')
            # plt.savefig(file_path)
            ...
            # plt.title('actual y vs predicted y' + str(dataset) + str(int(rate) / int(100)) + str(model_name) + 'R2' + '{:.2}'.format(r2))
            # plt.legend()
            # folder_path = 'E:/class/Ewh, University!/8/summer project/sina TDM/Hoor result' + '/' + model_name + '/' + 'one layer 95 97' + '/' + str(
            #     dataset) + '/' + 'prepared_data-' + str(num)
            # os.makedirs(folder_path, exist_ok=True)
            # file_path = os.path.join(folder_path, str(dataset) + str(rate) + '.png')
            # plt.savefig(file_path)
    ...
    def process_and_save_data(metric, data):
        ...
        directory_path = f'E:/class/Ewh, University!/8/summer project/sina TDM/Hoor result/test metrics95 97 GAT/{dataset}/'
