...
class Machine_Engine:
    ...
    def predict(self, x: np.ndarray, features_data: list, y_scaler, x_scaler=None, reset: bool = True) -> pd.DataFrame:
        """
        Predicts the output for the given data using the trained model.

        Parameters:
            x (np.ndarray): The data to be predicted.
            features_data (list): list of features for prediction.
            ...
            reset (bool): if you want to reset all previous predictions. Default is True.
        ...
        x_scaled = torch.tensor(x_scaled, dtype=torch.float32).unsqueeze(dim=0).to(self.device)
        ...
        if self.prediction is None or reset:
        ...
        new_x = np.concatenate((x.values[1:], np.concatenate((y_predict, [features.pop(0)]), axis=1)), axis=0)
        self.predict(x=new_x, features_data=features, y_scaler=y_scaler, x_scaler=x_scaler, reset=False)
        return self.prediction
    ...
    def plot_loss(self, save: bool = False):
        ...
        if save:
            plt.savefig(
                f"plot/{Information.model_name}_{Information.model_architecture}_{Information.model_version}_loss.png")
        plt.show()

    def plot_acc(self, save: bool = False):
        plt.figure(figsize=(10, 5))
        ...
        if save:
            plt.savefig(
                f"plot/{Information.model_name}_{Information.model_architecture}_{Information.model_version}acc.png")
        plt.show()

    def plot_predict_real(self, save: bool = False):
        ...
        plt.figure(figsize=(10, 5))
        plt.plot(x1, y1, label="Train Output", c="orange")
        plt.plot(x1+x2, r1+r2, label="Actual Output", c="b")
        plt.plot(x2, y2, label="Predicted Output", c="r")
        print(len(x1+x2))
        plt.ylabel("Health Tourist")
        plt.xlabel("Data")
        # plt.plot(x2, r2, label="Actual Output", c="b")
        plt.legend()
        if save:
            plt.savefig(
                f"plot/{Information.model_name}_{Information.model_architecture}_{Information.model_version}_compare.png")
        plt.show()
    ...
    def save(self):
        ...
        save_path1 = f"model/{Information.model_architecture}/{Information.model_name}_{Information.model_architecture}_{Information.model_version}2.pt"
