...
class Machine_Engine:
    def __init__(
        ...
        self.device = None
        (
            self.train_true_predict_list,
            self.val_true_predict_list,
            self.test_true_predict_list,
        ) = (None, None, None)

    def _train_step(
        self, loss_fn: torch.nn.Module, optimizer: torch.optim.Optimizer
    ) -> tuple:
        """
        ...
        for batch, (x, y) in enumerate(self.train_dataloader):
            ...
            true_predict_list["predict"].append(
                y_pred_labels.squeeze().detach().cpu().numpy()
            )
    ...
    def _val_step(self, loss_fn: torch.nn.Module) -> tuple:
        ...
        with torch.inference_mode():
            for x, y in self.val_dataloader:
                ...
                true_predict_list["predict"].append(
                    val_pred_labels.detach().cpu().numpy()
                )
    ...
    def train(
        ...
        try:
            if writer:
                writer = Machine_Engine._create_writer(
                    model_name=model_name, epochs=str(epochs_num)
                )
        except:
            raise EnvironmentError(
                "No writer found. Please check your torch.utils.tensorboard.SummaryWriter()"
            )

        self.device = device
        self.model.to(device)
        for epoch in tqdm(range(epochs_num)):
            train_loss, train_acc, true_predict_list = self._train_step(
